...
parser = argparse.ArgumentParser(
    description="Create an animation of density perturbation from multiple simulation outputs.",
...
for istep in range(step_s, step_e + 1):
    ...
    for idir, dirname in enumerate(dirnames):
        time, x, den = read_snapshot(dirname, istep)

        if time_ref is None:
            time_ref = time
            x_ana = np.linspace(-0.5, 0.5, 128)
            pg_exact, = ax[0].plot( x_ana, 1e-5 * np.sin(2.0 * np.pi * (x_ana - time_ref)),\
                                  '-', c="k", lw=1.5)
            artists.append(pg_exact)

        color = colors[idir % len(colors)]
        pg_num, = ax[0].plot( x, den - 1.0, 'o-', c=color, mfc="none")
        artists.append(pg_num)
    ...
    pg_time = ax[0].text( 0.5, 1.02, r"$\mathrm{time} = %.2f$" % time_ref, horizontalalignment="center",\
                          transform=ax[0].transAxes)
    artists.append(pg_time)
