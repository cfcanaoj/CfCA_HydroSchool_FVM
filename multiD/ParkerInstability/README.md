...
$$
    \bm{g} = \left(0, - g_0 \tanh(y/H_g)\right)
$$
...
$$
    \bm{B} = (B_x(y),0,0),\;\;\;\beta_0 = \frac{2P(y)}{B_x(y)^2} = \mathrm{const.}
$$

鉛直方向の静水圧平衡の式は，

$$
\frac{d}{dy}\left( P + \frac{B_x^2}{2}\right) = \rho g_y\;\;\;
\Longrightarrow\;\;\;
...
この式を使うと，静水圧平衡の式は，圧力に関する常微分方程式となるので，数値的に積分できる。式(\ref{parker_hydrostatic})から以下の形式解が得られる。
