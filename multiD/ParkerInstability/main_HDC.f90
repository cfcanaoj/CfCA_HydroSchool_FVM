...
real(8), parameter:: timemax=60.0d0 ! simulation end time
...
!character(20),parameter::dirname="hdc_gam1.4" ! directory name
...
real(8), external :: TimestepControl
      ...
      call GenerateProblem(xv, yv, yf, Q )
      call Prim2Consv(Q, U)
      call BoundaryCondition(yv,Q)
...
! main loop
      ...
      mloop: do !ntime=1,ntimemax
         ...
         call SrcTerms( 0.5d0*dt, dt, yv, Q, U)
         call Consv2Prim( U, Q )
         call BoundaryCondition(yv,Q )
!
         call NumericalFlux( dt, xf, yf, Q, F, G )
         call UpdateConsv( dt, xf, yf, F, G, Uo, U )
         call SrcTerms( dt, dt, yv, Q, U)
         call Consv2Prim( U, Q )
         call BoundaryCondition(yv,Q )
...
!=============================================================
subroutine GenerateProblem( xv, yv, yf, Q )
...
!=============================================================
subroutine BoundaryCondition(yv,Q)
use params, only : nxtot, nytot, NVAR, ngh, is, ie, js, je, g0, &
                   IDN, IVX, IVY, IVZ, IPR, IBX, IBY, IBZ, IPS, gam, TH
implicit none
...
!          Q(IDN,i,js-j)  = Q(IDN,i,js-1+j)
          ...
          Q(IPR,i,js-j)  = exp( log(Q(IPR,i,js)) &
                                  + 0.5*g0*(yv(js-j) - yv(js))/TH)
!          Q(IPR,i,js-j)  = exp( log(Q(IPR,i,js-1+j)) &
!                                  - g0*(2*j-1)*(yf(j+1)-yf(j))/TH)

          Q(IBX,i,js-j)  = sqrt(2.0*Q(IPR,i,js-j))
          Q(IBY,i,js-j)  = Q(IBY,i,js-1+j)
          Q(IBZ,i,js-j)  = Q(IBZ,i,js-1+j)
          Q(IPS,i,js-j)  = Q(IPS,i,js-1+j)
...
!          Q(IDN,i,je+j) = Q(IDN,i,je-j+1)
          ...
          Q(IPR,i,je+j) = exp( log(Q(IPR,i,je)) &
                                  - 0.5*g0*(yv(je+j) - yv(je))/TH)
          Q(IBX,i,je+j)  = sqrt(2.0*Q(IPR,i,je+j))
          Q(IBY,i,je+j) = Q(IBY,i,je-j+1)
          Q(IBZ,i,je+j) = Q(IBZ,i,je-j+1)
          Q(IPS,i,je+j) = Q(IPS,i,je-j+1)
      enddo
      enddo
!$omp end do
...
!-------------------------------------------------------------------
subroutine SrcTerms( dt1, dt0, yv, Q, U )
use params, only : IDN, IVX, IVY, IVZ, IPR, IBX, IBY, IBZ, &
                   IMX, IMY, IMZ, IEN, IPS, NVAR, nxtot, nytot, &
                   is, ie, js, je, gam, alpha, Ccfl, g0, Hg
implicit none
...
use params, only : IDN, IVX, IVY, IVZ, IPR, IBX, IBY, IBZ, IPS, NVAR, & 
                   nxtot, nytot, nx, ny, is, ie, js, je, ngh, gam, &
...
!          tot = tot + Q(IVY,i,j)**2
      ...
      enddo
      enddo
      phys_evo(1:nevo) = sqrt(tot/((ie-is+1)*(je-js+1)))
