...
use params, only : IDN, IVX, IVY, IVZ, IPR, IBX, IBY, IBZ, &
                   IPS, NVAR, nxtot, nytot, &
                   is, ie, js, je, g0, beta0, TH, ngh
implicit none
...
do i=1,nxtot-1
  do ihy=1,NVAR
     ...
     Q(IPR,i,js-j)  = exp( log(Q(IPR,i,js)) &
                             + 0.5*g0*(yv(js-j) - yv(js))/TH)
     Q(IBX,i,js-j)  = sqrt(2.0*Q(IPR,i,js-j)/beta0)
     ...
     Q(IPR,i,je+j) = exp( log(Q(IPR,i,je)) &
                        - 0.5*g0*(yv(je+j) - yv(je))/TH)
     Q(IBX,i,je+j)  = sqrt(2.0*Q(IPR,i,je+j)/beta0)
