...
real(8), parameter :: alpha = 0.01d0    ! decay timescale of divergence B
...
external :: NumericalFlux, UpdateConsv, SrcTerms, Consv2Prim
  ...
  call GenerateProblem(xf, xv, yf, yv, Q )
...
!=============================================================
subroutine GenerateProblem( xf, xv, yf, yv, Q )
use params, only : IDN, IVX, IVY, IVZ, IPR, IBX, IBY, IBZ, &
                   IMX, IMY, IMZ, IEN, IPS, NVAR, nxtot, nytot, &
                   is, ie, js, je, gam, nxtot, nytot, xmin, xmax, ymin, ymax
implicit none
...
real(8) :: bphi1(nxtot,nytot) ! vector potential at i,j-1/2
real(8) :: bphi2(nxtot,nytot) ! vector potential at i-1/2,j
...
real(8) :: psinorm, rand, v0,pi, B0
     ...
     &                        * sin(k_ini*yv(j)*2.0d0*pi/(ymax-ymin))
         ...
         bphi1(i,j) = psinorm * sin(k_ini*xv(i)*2.0d0*pi/(xmax-xmin)) &
     &                        * sin(k_ini*yf(j)*2.0d0*pi/(ymax-ymin))
         bphi2(i,j) = psinorm * sin(k_ini*xf(i)*2.0d0*pi/(xmax-xmin)) &
     &                        * sin(k_ini*yv(j)*2.0d0*pi/(ymax-ymin))
      enddo
      enddo
      ...
      do i=is,ie
         ...
         Q(IPR,i,j) = 1.0d0
         Q(IBX,i,j) =   B0*(bphi1(i,j+1)-bphi1(i,j))/(yf(j+1)-yf(j))
         Q(IBY,i,j) = - B0*(bphi2(i+1,j)-bphi2(i,j))/(xf(i+1)-xf(i))
         Q(IBZ,i,j) = 0.0d0
         Q(IPS,i,j) = 0.0d0
      enddo
      enddo
...
integer :: i,j
! NOTE:
!   Ql/Qr are large work arrays. Make them SAVEd so that we do NOT create
!   a private copy per thread when this subroutine is called inside a single
!   long-lived OpenMP parallel region.
real(8), save :: Ql(NVAR,nxtot,nytot), Qr(NVAR,nxtot,nytot)
...
!$omp do collapse(2) schedule(static) private(i,j,flx)
do j=js,je
do i=is,ie+1
  if (flag_flux == 1) then
...
real(8) :: dx,dy;
    ...
    do j=js,je
    do i=is,ie
        Vor(i,j) = 0.5d0*(Q(IVY,i+1,j) - Q(IVY,i-1,j))/dx - 0.5d0*(Q(IVX,i,j+1) - Q(IVX,i,j-1))/dy;
        Az(i,j)  = 0.5d0*(Q(IBY,i+1,j) - Q(IBY,i-1,j))/dx - 0.5d0*(Q(IBX,i,j+1) - Q(IBX,i,j-1))/dy;
    enddo
    enddo
    ...
    if( flag_binary ) then
        ...
        write(unitsnap,*) "#nx, ny = ", nx, ny
          do j=js,je
          do i=is,ie
              write(unitsnap,'(1p,13(es24.16,1x))') xv(i), yv(j), Q(IDN,i,j), Q(IVX,i,j), Q(IVY,i,j), Q(IVZ,i,j), &
                                Q(IPR,i,j), Q(IBX,i,j), Q(IBY,i,j), Q(IBZ,i,j), Vor(i,j), Az(i,j), Q(IPS,i,j) 
