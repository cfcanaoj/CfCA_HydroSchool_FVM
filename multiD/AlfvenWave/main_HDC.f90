...
!  write(*,'(A,F10.6,A)') "elapsed = ", (t1 - t0), " s"
   ...
   call AnalysisAfterSimu(time,xv,yv,Q)
...
real(8) :: cosa, sina, pi, x1m, Bpara, ca, kwave, omega, amp
    ...
    do i=is,ie
          ...
          Q(IBX,i,j) = Bpara*cosa + amp*dsin(k_dot_x)*sina
          Q(IBY,i,j) = Bpara*sina - amp*dsin(k_dot_x)*cosa
          Q(IBZ,i,j) = - amp*dcos(k_dot_x)
    enddo
    enddo
...
!       AnalysisAfterSimu 
!-------------------------------------------------------------------
subroutine AnalysisAfterSimu(time,xv,yv,Q)
...
real(8) :: cosa, sina, pi, x1m, Bpara, ca, kwave, omega, amp
    ...
    do i=is,ie
          ...
          L1 = L1   &
             + abs( Q(IBX,i,j) - (Bpara*cosa + amp*dsin(k_dot_x)*sina) ) &
             + abs( Q(IBY,i,j) - (Bpara*sina - amp*dsin(k_dot_x)*cosa) )  &
             + abs( Q(IBZ,i,j) + amp*dcos(k_dot_x) ) 

          L2 = L2 &
             + ( Q(IBX,i,j) - (Bpara*cosa + amp*dsin(k_dot_x)*sina) )**2  &
             + ( Q(IBY,i,j) - (Bpara*sina - amp*dsin(k_dot_x)*cosa) )**2  &
             + ( Q(IBZ,i,j) + amp*dcos(k_dot_x) )**2 
    enddo
    enddo
