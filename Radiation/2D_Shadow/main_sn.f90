...
module gridmod
  ...
  integer, parameter :: jzones = 12*4
  integer, parameter :: kzones = 0
  ...
  subroutine GenerateProblem()
    ...
    do i=is,ie
       ...
       kappa(i,j,k) = 0.1 *(tempK(i,j,k)/tempMed)**(-3.5d0) &
            &                    *(    d(i,j,k)/rho0   )**2/d(i,j,k)    ! [cm^2/g] absorption opacity per mass ei(i,j,k)   = d(i,j,k)*Cv*tempK(i,j,k)
    ...
    do i=is,ie
        do m=1,mang
           fdis(m,i,j,k) = Erad(i,j,k)/dth(m)
           flte(m,i,j,k) = arad*(tempK(i,j,k))**4/dth(m) ! [erg/cm^3]
       enddo
    enddo
    enddo
    enddo
...
subroutine RadBoundaryCondition
  ...
  case(grid_aligned)
     ...
     do j=js,je
     do i=1,mgn
        fdis(1     ,is-i,j,k) = arad*tempRad**4/dth(1)
        fdis(2:mang,is-i,j,k) = arad*tempMed**4/dth(2:mang)
     enddo
     enddo
     enddo
  case(staggered)
     ...
     do i=1,mgn
        fdis(1       ,is-i,j,k) = arad*tempRad**4/dth(1)
        fdis(2:mang-1,is-i,j,k) = arad*tempMed**4/dth(2:mang-1)
        fdis(mang    ,is-i,j,k) = arad*tempRad**4/dth(mang)
     enddo
     enddo
     enddo
end select
...
! j-direction
  ...
  do j=1,mgn
     do m=1,mang
        fdis(m,i,js-j,k) = fdis(m,i,js+j-1,k)
     enddo
  enddo
  enddo
  enddo
...
!$omp parallel do default(shared) private(i,j,k,m,alpha) collapse(3) schedule(static)
    ...
    do j=js,je
    do i=is,ie
         kappa(i,j,k) = 0.1d0 *(tempK(i,j,k)/tempMed)**(-3.5d0) &
    &                       *(    d(i,j,k)/rho0   )**2/d(i,j,k)    ! [cm^2/g] absorption opacity per mass alpha = d(i,j,k)*kappa(i,j,k) *cl*dt
