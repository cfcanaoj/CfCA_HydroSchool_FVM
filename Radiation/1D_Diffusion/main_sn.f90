...
module statemod
  ...
  real(8),dimension(in,jn,kn):: flte
...
!$omp parallel do default(shared) private(i,j,k,m,alpha) collapse(3) schedule(static)
    ...
    do i=is,ie
       fiso = 0.0d0
       do m=1,mang
          fiso = fiso + fstar(m,i,j,k)*dth(m) 
       enddo
       fiso = fiso/sum(dth(:))

       alpha = d(i,j,k)*kappaA(i,j,k) *cl*dt
       fiso = (fiso+alpha*flte(i,j,k))/(1+alpha) 

       beta = d(i,j,k)*kappaS(i,j,k) *cl*dt
         do m=1,mang
            fdis(m,i,j,k) = (fstar(m,i,j,k) + alpha* flte(i,j,k) + beta*fiso)/(1.0d0+alpha+beta)
         enddo
    enddo
    enddo
    enddo
