...
!$acc end kernels
      ...
      implicit none
      integer::i,j,k
      character(20),parameter::dirname="hyd/"
...
!$acc update host (d,v1,v2,v3,p)
      ...
      open(unitout,file=filename,status='replace',form='formatted',action="write") 

      write(unitout,"((a1,1x),E12.3)") "#",time
      write(unitout,"((a1,1x),i0)") "#",ie-is+1
      write(unitout,"((a1,1x),i0)") "#",je-js+1
      write(unitout,"(A)") "# x y d v1 v2 v3 p"
      
      k=ks
      do j=js,je
      do i=is,ie
         write(unitout,'(7(E12.3,1x))') x1b(i),x2b(j),d(i,j,k),v1(i,j,k),v2(i,j,k),v3(i,j,k),p(i,j,k)
      enddo
         write(unitout,*)
      enddo
      close(unitout)
